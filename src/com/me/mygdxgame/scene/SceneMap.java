...
public class SceneMap extends SceneBase implements InputProcessor{
	...
	public SceneMap(int[] tilemapSize){
		Game.map.setup(tilemapSize);
	...
	public boolean touchDragged(int x, int y, int pointer) {
		if(Gdx.input.isButtonPressed(Input.Buttons.RIGHT) || Gdx.input.isTouched(1)){
			...
			Intersector.intersectRayPlane(pickRay, GameMap.xyPlane, curr);
	 
			if(!(last.x == -1 && last.y == -1 && last.z == -1)) {
				pickRay = Game.cam.getPickRay(last.x, last.y);
				Intersector.intersectRayPlane(pickRay, GameMap.xyPlane, delta);			
				delta.sub(curr);
